# 翻墙指南 (Fanqiang Wiki)

[![Docusaurus](https://img.shields.io/badge/Built%20with-Docusaurus-3ECC5F.svg)](https://docusaurus.io/)
[![Node](https://img.shields.io/badge/Node-%3E%3D18.0-brightgreen.svg)](https://nodejs.org/)

> 小白翻墙指南：客户端、VPN、机场、自建代理服务器完整教程，帮助您安全稳定地访问国外网站

🌐 **Website**: [https://www.fanqiang.wiki](https://www.fanqiang.wiki)

> 网站依旧处于建设中。内容尚不完善

## 📖 简介

本项目是一个面向小白的科学上网完整指南，涵盖关于翻墙所需要知道的一切：
- 🛠️ 自建代理服务器教程
- 📱 各平台代理客户端使用说明
- 🔐 VPN选择
- ✈️ 机场选择

## ✨ 特性

- **全面的内容覆盖**: 从入门到进阶，包含VPN、机场、自建节点等多种翻墙方式
- **详细的教程**: 提供视频教程，适合零基础用户
- **客户端对比**: 详细对比各平台代理客户端的特点和使用方法
- **实时更新**: 持续更新最新的翻墙技术和工具推荐
